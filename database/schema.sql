...
-- Create "users" table
CREATE TABLE "public"."users" ("id" uuid NOT NULL, "username" character varying(255) NOT NULL, "password_hash" character varying(255) NULL, "email" character varying(255) NULL, "phone_number" character varying(15) NULL, "gender" character(1) NULL, "role" integer NOT NULL DEFAULT 1, "date_of_birth" date NULL, "location" uuid NULL, "created_at" timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, "updated_at" timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY ("id"), CONSTRAINT "users_email_key" UNIQUE ("email"), CONSTRAINT "users_phone_number_key" UNIQUE ("phone_number"), CONSTRAINT "users_username_key" UNIQUE ("username"), CONSTRAINT "users_location_fkey" FOREIGN KEY ("location") REFERENCES "public"."locations" ("id") ON UPDATE NO ACTION ON DELETE SET NULL, CONSTRAINT "users_check" CHECK (((email IS NOT NULL) AND (char_length((email)::text) > 0)) OR ((phone_number IS NOT NULL) AND (char_length((phone_number)::text) > 0))), CONSTRAINT "users_email_check" CHECK (char_length((email)::text) > 0), CONSTRAINT "users_gender_check" CHECK (gender = ANY (ARRAY['M'::bpchar, 'F'::bpchar])), CONSTRAINT "users_phone_number_check" CHECK (char_length((phone_number)::text) > 0), CONSTRAINT "users_username_check" CHECK (char_length((username)::text) > 0));
