...
-- Create "users" table
CREATE TABLE "public"."users" ("id" uuid NOT NULL, "username" character varying(255) NOT NULL, "password_hash" character varying(255) NULL, "email" character varying(255) NULL, "phone_number" character varying(15) NULL, "gender" character(1) NULL, "is_admin" boolean NOT NULL DEFAULT false, "date_of_birth" date NULL, "location" uuid NULL, "created_at" timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, "updated_at" timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY ("id"), CONSTRAINT "users_email_key" UNIQUE ("email"), CONSTRAINT "users_phone_number_key" UNIQUE ("phone_number"), CONSTRAINT "users_username_key" UNIQUE ("username"), CONSTRAINT "users_location_fkey" FOREIGN KEY ("location") REFERENCES "public"."locations" ("id") ON UPDATE NO ACTION ON DELETE SET NULL, CONSTRAINT "users_check" CHECK (((email IS NOT NULL) AND (char_length((email)::text) > 0)) OR ((phone_number IS NOT NULL) AND (char_length((phone_number)::text) > 0))), CONSTRAINT "users_email_check" CHECK (char_length((email)::text) > 0), CONSTRAINT "users_gender_check" CHECK (gender = ANY (ARRAY['M'::bpchar, 'F'::bpchar])), CONSTRAINT "users_phone_number_check" CHECK (char_length((phone_number)::text) > 0), CONSTRAINT "users_username_check" CHECK (char_length((username)::text) > 0));
...
CREATE TABLE "public"."product_category" ("id" uuid NOT NULL, "name" character varying(255) NOT NULL, "parent_id" uuid NULL, PRIMARY KEY ("id"), CONSTRAINT "product_category_name_key" UNIQUE ("name"), CONSTRAINT "product_category_parent_id_fkey" FOREIGN KEY ("parent_id") REFERENCES "public"."product_category" ("id") ON UPDATE NO ACTION ON DELETE CASCADE, CONSTRAINT "product_category_name_check" CHECK ((char_length((name)::text) > 0) AND (char_length((name)::text) < 32)));
...
CREATE TABLE "public"."products" ("id" uuid NOT NULL, "product_name" character varying(255) NULL, "category_id" uuid NOT NULL, PRIMARY KEY ("id"), CONSTRAINT "products_product_name_key" UNIQUE ("product_name"), CONSTRAINT "products_category_id_fkey" FOREIGN KEY ("category_id") REFERENCES "public"."product_category" ("id") ON UPDATE NO ACTION ON DELETE NO ACTION, CONSTRAINT "products_product_name_check" CHECK (char_length((product_name)::text) > 0));
...
CREATE TABLE "public"."attributes" ("id" uuid NOT NULL, "attribute_name" character varying(255) NOT NULL, "attribute_value" character varying(255) NOT NULL, PRIMARY KEY ("id"));
